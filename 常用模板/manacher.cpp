// Computes the radius of the longest palindrome centered at each element in
// O(n) total. Actual palindrome length is `2 * radius[i] + 1`.
...
vector<int> odd_manacher(const T_string &pattern) {
  ...
  for (int i = 1; i < n; i++) {
    ...
    while (i - radius[i] > 0 && i + radius[i] < n - 1 &&
           pattern[i - radius[i] - 1] == pattern[i + radius[i] + 1])
      radius[i]++;
...
// Returns a vector `lengths` of size `2 * n + 1` where `lengths[i]` indicates a
...
// Returns whether the substring represented by `[start, end)` is a palindrome
// using the output of `combined_manacher`.
